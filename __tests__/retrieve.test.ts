...
describe('retrieve.ts', () => {
  ...
  it('should properly handle SF_USERNAME and SF_PASSWORD from environment variables', () => {
    ...
    expect(sourceCode).toContain("password: (process.env.SF_PASSWORD || '') + (process.env.SF_TOKEN || \"\")");
  });
  
  it('should extract zip files if present', () => {
    // Check for zip extraction logic
    expect(sourceCode).toContain("const zipFiles = fs.readdirSync(retrieveTargetDir).filter(file => file.endsWith('.zip'))");
